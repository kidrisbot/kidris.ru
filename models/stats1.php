...
//проверить, куда это идет?
  ...
  $rowCount == 3;
    ...
    foreach ( $_SESSION['groups'] as $id => $group) {
      ...
      $spisoktpl .= '<li ' . $classActive . '><a href="//kidris.ru/dashh/' . $group['id'] . '" >' . htmlspecialchars($groupname) . '</a></li>';
...
$vars += [
 ...
 $vars['url'] = 
             '  
        <p><a class="btn btn-warning" id="but_back_'.$rowCount2.'" onclick="stats_get(0, \''.$rowCount2.'\', '.$rowCount2.')">Более новые записи</a>
      <a class="btn btn-info" id="but_next_'.$rowCount2.'" onclick="stats_get(20, \''.$rowCount2.'\', '.$rowCount2.')">Более старые записи</a></p>
      ';
